...
//Comprueba si existen campos vacíos
	if(empty($marca) || empty($color) || empty($refmotor) || empty($matricula)) 
	{
		if(empty($marca)) {
...
//Prepara una sentencia SQL para su ejecución. En este caso el alta de un registro de la BD.		
		$stmt = mysqli_prepare($mysqli, "INSERT INTO cars (idmarca,color,refmotor,matricula) VALUES(?,?,?,?)");
...
*/		
		mysqli_stmt_bind_param($stmt, "isss", $marca, $color, $refmotor, $matricula);
